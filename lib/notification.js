 ...
 * @param {string} html
 */
export async function sendResult(target, html) {
  const $ = await parseAsync(html);
  const result = $('textarea').text();
  console.log(target.course, '抢课结果', result);
...
- ${time}
-`, '')}`;
    const template_id = err.caurse === null ? elect.success_template_id : elect.failure_template_id;
